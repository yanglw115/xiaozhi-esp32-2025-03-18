...
VeRtcProtocol::~VeRtcProtocol() {
    ESP_LOGI(TAG, "VeRtcProtocol deinit");
    if (udp_ != nullptr) {
        delete udp_;
    }
    if (vertc_ != nullptr) {
...
void VeRtcProtocol::Start() {
    StartVeRtcClient(false);
}

bool VeRtcProtocol::StartVeRtcClient(bool report_error) {
    if (vertc_ != nullptr) {
        ESP_LOGW(TAG, "VeRtc client already started");
        delete vertc_;
    }
    ...
    vertc_ = Board::GetInstance().CreateVeRtc();
    vertc_->SetKeepAlive(90);

    vertc_->OnDisconnected([this]() {
        ESP_LOGI(TAG, "Disconnected from endpoint");
    });

    vertc_->OnMessage([this](const std::string& topic, const std::string& payload) {
    ...
    if (!vertc_->Connect(endpoint_, 8883, client_id_, username_, password_)) {
...
void VeRtcProtocol::SendText(const std::string& text) {
    ...
    if (!vertc_->Publish(publish_topic_, text)) {
...
bool VeRtcProtocol::OpenAudioChannel() {
    if (vertc_ == nullptr || !vertc_->IsConnected()) {
        ESP_LOGI(TAG, "MQTT is not connected, try to connect now");
        if (!StartVeRtcClient(true)) {
            return false;
        }
    }
...
std::string VeRtcProtocol::DecodeHexString(const std::string& hex_string) {
    std::string decoded;
